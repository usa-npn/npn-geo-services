...
async function getClippedSixImage(boundary, date, plant, phenophase, climate) {
    ...
    let query = `SELECT
    ST_AsTIFF(ST_SetBandNoDataValue(ST_Union(ST_Clip(r.rast, ST_Buffer(foo.boundary, ${buffer}), -9999, true)), 1, null)) AS tiffy
    FROM (SELECT p.gid as gid, p.geom AS boundary FROM fws_boundaries p WHERE p.orgname = '${boundary}') as foo
    INNER JOIN ${rastTable} r ON ST_Intersects(r.rast, foo.boundary)
    AND r.rast_date = '${dateString}'
    AND r.plant='${plant}'
...
async function saveSixAreaStatsToCache(boundary, plant, phenophase, climate, dateString, count, mean, stddev, min, max, percent_complete) {
    ...
    let query2 = `INSERT INTO cached_six_area_stats (boundary, plant, phenophase, climate, date, count, mean, stddev, min, max, percent_complete)
