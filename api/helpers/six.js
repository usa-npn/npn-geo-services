...
FROM (
    SELECT ST_Clip(ST_Union(r.rast), ST_Buffer(ST_Union(${useConvexHullBoundary ? 'foo.convex_hull_boundary' : 'foo.boundary'}), $1), -9999, true) AS clipped_raster
    FROM
    (
        SELECT ST_Union(ST_MakeValid(p.geom)) AS boundary,
        ST_ConvexHull(ST_Union(ST_MakeValid(p.geom))) AS convex_hull_boundary
