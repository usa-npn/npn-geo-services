//misc scripts for boundary processing
...
async function dropConvexHullBoundaryTable() {
    let query = `DROP TABLE fws_boundaries_convexhull;`;
    const res = await db.pgPool.query(query);
    return res;
}

async function convertBoundaryToConvexHull(refugeName) {
    let query = `
    UPDATE fws_boundaries_convexhull SET geom = (SELECT ST_Multi(ST_ConvexHull(ST_MakeValid(ST_Union(p.geom))))
    FROM fws_boundaries_convexhull p
    WHERE p.orgname = '${refugeName}') WHERE orgname = '${refugeName}';
    `;
    const res = await db.pgPool.query(query);
    return res;
}

async function deleteDuplicateRows() {
    let query = `
    DELETE t1 FROM fws_boundaries_convexhull t1
    JOIN fws_boundaries_convexhull t2
    ON t2.orgname = t1.orgname
    AND t2.gid < t1.gid
    `;
    const res = await db.pgPool.query(query);
    return res;
...
async function convertToConvexHull() {
    //await dropConvexHullBoundaryTable();
    exec( 'shp2pgsql -I -s 4269 FWS_boundaries_simplified_RSLdiss.shp fws_boundaries_convexhull | psql -U postgres -d climate', async (error, stdout, stderr) => {
        if (error) {
            console.error(`exec error: ${error}`);
            return;
        }
        for (var boundary of fwsBoundaries) {
            console.log(`converting boundary: ${boundary}`);
            await convertBoundaryToConvexHull(boundary);
        }
